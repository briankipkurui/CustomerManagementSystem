...
function CustomerDrawerForm({showDrawer,setShowDrawer,fetAllCustomers}) {
    const onCLose = () => setShowDrawer(false);
    const [submitting, setSubmitting] = useState(false);
    let{authTokens} = useContext(AuthContext)

    const addNewCustomer = customer =>
        fetch("management/api/v1/customer/registerUser", {
        ...
        addNewCustomer(customer)
            .then(() =>{
                ...
                successNotification(
                    ...
                    `${customer.firstName} was  added to the system`
                )
                fetAllCustomers();
            }).catch(err => {
            console.log(err);
