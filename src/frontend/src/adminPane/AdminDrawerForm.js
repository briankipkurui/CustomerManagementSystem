...
function AdminDrawerForm({showAdminDrawer,setShowAdminDrawer,fetAllCustomers}) {
    const onCLose = () => setShowAdminDrawer(false);
    const [submitting, setSubmitting] = useState(false);
    let{authTokens} = useContext(AuthContext)

    const addNewAdmin = customer =>
        fetch("management/api/v1/customer/registerAdmin", {
        ...
        addNewAdmin(admin)
            .then(() =>{
                ...
                successNotification(
                    ...
                    `${admin.firstName} ${admin.lastName}was  added to the system`
                )
                fetAllCustomers();
            }).catch(err => {
            console.log(err);
